...
let grid;
let tam = 10; // Largura de cada quadrado em px
let coluna, linha;
let hueValue = 200; // Cor da areia

function setup() { // Cria o canvas 800px por 800px e zera todos os valores do grid
...
function mouseDragged() {
    ...
    hueValue += 0.5; // Vai mudando a cor conforme o mouse for segurado
    if (hueValue > 360) { // Se chegar no valor máximo volta ao valor da cor inicial em RGB
        hueValue = 1;
    }
}
...
function draw() {
    ...
    for (let i = 0; i < coluna; i++) { // Simulação da "gravidade" da areia
        for (let j = 0; j < linha; j++) {
            let valor = grid[i][j];
            if (valor > 0) {
